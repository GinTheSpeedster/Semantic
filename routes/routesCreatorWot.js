...
function createRootRoute(model) {
  router.route('/').get(function (req, res, next) {
    ...
    req.model = model;
    req.type = 'root';
    req.typeJson = 'openApiTemplate.json';
...
function createPropertiesRoutes(model) {
  ...
  router.route(properties.link + '/:id').get(function (req, res, next) {
    ...
    req.type = 'property';
	req.typeld = 'propertyld.json';
    req.entityId = req.params.id;

    req.result = reverseResults(properties.resources[req.params.id].data);
    if(req.result[0]) {
      req.value = req.result[0][Object.keys(req.result[0])[0]];
    }
	req.context = 'http://schema.org';


	
	if(req.params.id == 'temperature'){
		req.unit = 'CEL';
	}
	else if (req.params.id == 'humidity'){
		req.unit = 'P1';
	}
	else if (req.params.id == 'airpressure'){
		req.unit = 'A97';
	}
	else if (req.params.id == 'pir'){
		req.unit = '';
	}
	else {
		req.unit = '';
